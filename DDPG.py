...
TEST_PER_EPISODES = 10  # test the surrogate_model per episodes
...
class DDPG(object):
    ...
    def __init__(self, a_dim, s_dim, a_bound):
        ...
        def copy_para(from_model, to_model):
            ...
            :param from_model: latest surrogate_model
            :param to_model: target surrogate_model
            :return: None
            """
    ...
    def save_ckpt(self):
        ...
        :return: None
        """
        if not os.path.exists('surrogate_model'):
            os.makedirs('surrogate_model')

        tl.files.save_weights_to_hdf5('surrogate_model/ddpg_actor.hdf5', self.actor)
        tl.files.save_weights_to_hdf5('surrogate_model/ddpg_actor_target.hdf5', self.actor_target)
        tl.files.save_weights_to_hdf5('surrogate_model/ddpg_critic.hdf5', self.critic)
        tl.files.save_weights_to_hdf5('surrogate_model/ddpg_critic_target.hdf5', self.critic_target)
    ...
    def load_ckpt(self):
        ...
        :return: None
        """
        tl.files.load_hdf5_to_weights_in_order('surrogate_model/ddpg_actor.hdf5', self.actor)
        tl.files.load_hdf5_to_weights_in_order('surrogate_model/ddpg_actor_target.hdf5', self.actor_target)
        tl.files.load_hdf5_to_weights_in_order('surrogate_model/ddpg_critic.hdf5', self.critic)
        tl.files.load_hdf5_to_weights_in_order('surrogate_model/ddpg_critic_target.hdf5', self.critic_target)
